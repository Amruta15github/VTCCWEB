...
public partial class adminpanel_testimonials_master : System.Web.UI.Page
{
    ...
    private void FillGrid()
    {
        try
        {
            using (DataTable dtNws = c.GetDataTable("Select TestId, Convert(varchar(20), TestDate, 103) as TestDate, TestName, TestEmail, TestLocation from Testimonials Where DelMark=0  Order By TestId DESC"))
            {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lblId.Text == "[New]")
                {
                    c.ExecuteQuery("Insert into Testimonials(TestId, TestDate, TestName, TestEmail, TestLocation, TestDesc, delMark)Values(" + maxId + ", '" + DateTime.Now + "' ,'" + txtPerNm.Text + "', '" + txtEmail.Text + "', '" + txtPlce.Text + "', '" + txtTesDesc.Text + "',0)");

                    ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Testimonials  Added');", true);
                }
                else
                {
                    c.ExecuteQuery("Update Testimonials set TestDate='" + DateTime.Now + "' , TestName='" + txtPerNm.Text + "', TestEmail='" + txtEmail.Text + "', TestLocation='" + txtPlce.Text + "', TestDesc='" + txtTesDesc.Text + "' where testId=" + maxId);
